...
// Get all properties and filter for favorites
      ...
      const favoriteProperties = allProperties.filter(property =>
        favorites.some(fav => fav.propertyId === parseInt(property.id))
      )
      
      // Sort by date added to favorites (most recent first)
      const sortedProperties = favoriteProperties.sort((a, b) => {
        const aFav = favorites.find(fav => fav.propertyId === parseInt(a.id))
        const bFav = favorites.find(fav => fav.propertyId === parseInt(b.id))
        return new Date(bFav?.createdAt || 0) - new Date(aFav?.createdAt || 0)
      })
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-display font-bold text-gray-900 mb-2">Saved Properties
                                      </h1>
                    <p className="text-gray-600">
                        {savedProperties.length === 0 ? "No saved properties yet" : `${savedProperties.length} propert${savedProperties.length !== 1 ? "ies" : "y"} saved`}
                    </p>
                </div>
                {savedProperties.length > 0 && <Button
                    variant="outline"
                    onClick={handleClearAllFavorites}
                    className="text-error border-error hover:bg-error hover:text-white">Clear All
                                  </Button>}
            </div>
        </motion.div>
        {/* Content */}
        {savedProperties.length === 0 ? <motion.div
            initial={{
                opacity: 0,
                y: 20
            }}
